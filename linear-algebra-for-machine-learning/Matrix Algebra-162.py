...
matrix_33 = np.asarray([[1,2,3],
                        [4,5,6],
                        [7,8,9]],dtype=np.float32)
matrix_23 = np.asarray([[1,2,3],
...
def matrix_inverse_two(mat):
    ...
    if det == 0:
        ...
        mat = np.asarray([[mat[1,1],-mat[0,1]],
                     [-mat[1,0],mat[0,0]]],dtype=np.float32)
        return np.dot(1/det , mat)
...
vector_b = np.asarray([[-1000],
                       [-100]],dtype=np.float32)

inverse_matrix = np.linalg.inv(matrix_a)
solution_x = np.dot(inverse_matrix,vector_b)
